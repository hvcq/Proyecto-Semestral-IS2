...
class ModelUser():
    rol_actual = "desconocido"
    email_encuestado = "sin_email"
    @classmethod
    def login(self, user):
        try:
            ...
            if admin != None:
                ...
                if registrado != None:
                    ...
                    ModelUser.rol_actual = "registrado"
                    return user
                else:
                    encuestado = db.session.query(Encuestado).filter_by(email=user.email).first()
                    if encuestado != None:
                        user = User(0, encuestado.email, True, "anonimo", "encuestado")
                        ModelUser.rol_actual = "encuestado"
                        ModelUser.email_encuestado = encuestado.email
                        return user
                    else:
                        return None
        ...
        try:
            if ModelUser.rol_actual == "admin":
                ...
                if registrado != None:
                    ...
                    return None
            elif ModelUser.rol_actual == "encuestado":
                encuestado = db.session.query(Encuestado).filter_by(email=ModelUser.email_encuestado).first()
                if encuestado != None:
