...
data.dataSurvey.questions?.map((element) => {
  element.type === "alternativa"
    ? poolId.alternativa.push(element.id)
    : poolId.desarrollo.push(element.id);
});
...
if (
  data.selected.toLowerCase() === "preguntas" &&
  Object.keys(data.dataSurvey).length !== 0
) {
...
const deleteElement = function (event) {
  ...
  console.log(parent);
  const arrayElement = data.dataSurvey.questions.filter(
    (element) => element.id !== idElement || element.type !== typeElement
  );
...
const newId = function (type) {
  return type === "alternativa"
    ? Math.max(...poolId.alternativa) + 1
    : Math.max(...poolId.desarrollo) + 1;
};
...
const setToAlternative = function (event) {
  ...
  if (parent.getAttribute("alternative") === "false") {
    const [question] = data.dataSurvey.questions.filter(
      (question) =>
        question.id === parseInt(idParent) && question.type === "desarrollo"
    );
...
const deleteAlternative = function (event) {
  ...
  if (parent.childElementCount > 2) {
    ...
    const [option] = result.alternatives.filter(
      (val) => val.id === parseInt(idOpcion)
    );
...
const handleInputs = function (event) {
  ...
  if (target.id === "title") data.dataSurvey.title = target.value;
    ...
    const [result] = data.dataSurvey.questions.filter(
      (question) => question.id === idQuestion && question.type === type
    );
    ...
    const [result] = data.dataSurvey.questions.filter(
      (question) => question.id === idParent && question.type === "alternativa"
    );

    const [resultOption] = result.alternatives.filter(
